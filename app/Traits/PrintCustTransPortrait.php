...
/* $Id: PrintCustTransPortrait.php 4950 2012-02-22 06:26:38Z daintree $ */
...
if (isset($_GET['correspondingInvoiceNumber'])) {
    $correspondingInvoiceNumber = $_GET['correspondingInvoiceNumber'];
} elseif (isset($_POST['correspondingInvoiceNumber'])) {
    $correspondingInvoiceNumber = $_POST['correspondingInvoiceNumber'];
} else {
    $correspondingInvoiceNumber = '';
}
...
if (isset($PrintPDF)
    ...
    if ($InvOrCredit == 'Invoice') {
        $pdf->addInfo('Title', _('Sales Invoice') . ' PrintCustTransPortrait.php' . $FromTransNo . ' to ' . $_POST['ToTransNo']);
        $pdf->addInfo('Subject', _('Invoices from') . ' PrintCustTransPortrait.php' . $FromTransNo . ' ' . _('to') . ' ' . $_POST['ToTransNo']);
    } else {
        $pdf->addInfo('Title', _('Sales Credit Note'));
        $pdf->addInfo('Subject', _('Credit Notes from') . ' PrintCustTransPortrait.php' . $FromTransNo . ' ' . _('to') . ' ' . $_POST['ToTransNo']);
    }
    ...
    while ($FromTransNo <= filter_number_format($_POST['ToTransNo'])) {
        ...
        if (DB_error_no() != 1) {
            if (DB_num_rows($result) == 1) {
                ...
                $DefaultBankAccountNumber = _('Account:') . ' PrintCustTransPortrait.php' . $myrow['bankaccountnumber'];
                $DefaultBankAccountCode = _('Bank Code:') . ' PrintCustTransPortrait.php' . $myrow['bankaccountcode'];
            } else {
...
// gather the invoice data
        ...
        if ($InvOrCredit == 'Invoice') {
            ...
            $sql = "SELECT debtortrans.trandate,
                                                ...
                                                AND debtortrans.branchcode=custbranch.branchcode
                                                INNER JOIN salesman
                                                ON debtortrans.salesperson=salesman.salesmancode
                                                INNER JOIN paymentterms
                                                ON debtorsmaster.paymentterms=paymentterms.termsindicator
                                                INNER JOIN currencies
                                                ON debtorsmaster.currcode=currencies.currabrev
        ...
        if (DB_error_no() != 0) {
            ...
            prnMsg(_('There was a problem retrieving the invoice or credit note details for note number') . ' PrintCustTransPortrait.php' . $InvoiceToPrint . ' ' . _('from the database') . '. ' . _('To print an invoice, the sales order record, the customer transaction record and the branch record for the customer must not have been purged') . '. ' . _('To print a credit note only requires the customer, transaction, salesman and branch records be available'), 'error');
            if ($debug == 1) {
        ...
        if (DB_num_rows($result) == 1) {
            ...
            if (DB_num_rows($result) > 0) {
                ...
                while ($myrow2 = DB_fetch_array($result)) {
                    ...
                    $DisplayPrice = locale_number_format($myrow2['fxprice'] + ($myrow2['fxprice'] * (15 / 100)), $myrow['decimalplaces']);
                    //$DisplayPrice = locale_number_format($myrow2['fxprice'],$myrow['decimalplaces']);
                    $DisplayQty = locale_number_format($myrow2['quantity'], $myrow2['decimalplaces']);
                    //$DisplayNet = locale_number_format($DisplayPrice*$DisplayQty,$myrow['decimalplaces']);
                    $DisplayVat = locale_number_format((15 / 100) * $DisplayNet, $myrow['decimalplaces']);
                    $AmountIncl = locale_number_format($DisplayNet + $DisplayVat, $myrow['decimalplaces']);

                    $description = wordwrap($myrow2['description'],16,'\n','true')->explode("\n");

                    if ($myrow2['stockid'] != 'ROUNDING') {
                        $LeftOvers = $pdf->addTextWrap($Left_Margin, $YPos, 71, $FontSize, $myrow2['stockid']);
//                        $LeftOvers = $pdf->addTextWrap($Left_Margin + 80, $YPos, 186, $FontSize, $myrow2['description']);
                        $LeftOvers = $pdf->addTextWrap($Left_Margin + 80, $YPos, 186, $FontSize, array_pop($description));
                        $lines = 1;

                        if (sizeof($description)>0){
                            foreach ($description as $des){
                                $pdf->addTextWrap($Left_Margin + 80, $YPos - (10 * $lines), 186, $FontSize, $des);
                                $lines++;
                            }
                        }
                        else {
                        ...
                        $LeftOvers = $pdf->addTextWrap($Left_Margin + 190, $YPos, 76, $FontSize, $DisplayPrice, 'right');
                        $LeftOvers = $pdf->addTextWrap($Left_Margin + 290, $YPos, 36, $FontSize, $DisplayQty, 'right');
                        $LeftOvers = $pdf->addTextWrap($Left_Margin + 330, $YPos, 26, $FontSize, $myrow2['units'], 'center');
                        $LeftOvers = $pdf->addTextWrap($Left_Margin + 380, $YPos, 26, $FontSize, $DisplayDiscount, 'right');
                        $LeftOvers = $pdf->addTextWrap($Left_Margin + 370, $YPos, 26, $FontSize, $DisplayVat, 'right');
                        $LeftOvers = $pdf->addTextWrap($Left_Margin + 390, $YPos, 72, $FontSize, $DisplayNet, 'right');
                        $LeftOvers = $pdf->addTextWrap($Left_Margin + 450, $YPos, 72, $FontSize, $AmountIncl, 'right');
            ...
            if ($InvOrCredit == 'Invoice') {
                ...
                $DisplayTotal = locale_number_format($myrow['ovfreight'] + ($myrow['ovamount'] * (15 / 100)) + $myrow['ovamount'], $myrow['decimalplaces']);
            } else {
                $DisplaySubTot = locale_number_format(-$myrow['ovamount'], $myrow['decimalplaces']);
                $DisplayFreight = locale_number_format(-$myrow['ovfreight'], $myrow['decimalplaces']);
                //$DisplayTax = locale_number_format(-$myrow['ovgst'],$myrow['decimalplaces']);
                $DisplayTax = locale_number_format(-2, $myrow['decimalplaces']);
                $DisplayTotal = locale_number_format(-$myrow['ovfreight'] - $myrow['ovgst'] - $myrow['ovamount'], $myrow['decimalplaces']);
            }
            ...
            $pdf->addText($Page_Width - $Right_Margin - 220, ($YPos + 12) - (2 * $line_height) + 5, $FontSize, _('Total VAT'));
            $LeftOvers = $pdf->addTextWrap($Left_Margin + 450, $YPos - (2 * $line_height) + 5, 72, $FontSize, $DisplayTax, 'right');
            /* $pdf->addText($Page_Width-$Right_Margin-220, ($YPos+12)-(3*$line_height)+5,$FontSize, _('Total VAT'));
            $LeftOvers = $pdf->addTextWrap($Left_Margin+450,$YPos-(3*$line_height)+5,72, $FontSize,$DisplayTax, 'right');*/
            ...
            $YPos += 10;
            if ($InvOrCredit == 'Invoice') {
                $pdf->addText($Page_Width - $Right_Margin - 220, ($YPos + 6) - ($line_height * 3), $FontSize, _('TOTAL INVOICE') . ' PrintCustTransPortrait.php' . $myrow['currcode']);
                $FontSize = 8;
...
// Print Bank acount details if available and default for invoices is selected
                ...
                $pdf->addText($Page_Width - $Right_Margin - 220, ($YPos + 6) - ($line_height * 3), $FontSize, _('TOTAL CREDIT') . ' PrintCustTransPortrait.php' . $myrow['currcode']);
            }
    ...
    if (isset($_GET['Email'])) { //email the invoice to address supplied
        ...
        $mail->setText(_('Please find attached') . ' PrintCustTransPortrait.php' . $InvOrCredit . ' ' . $_GET['FromTransNo']);
        ...
        $Title = _('Emailing') . ' PrintCustTransPortrait.php' . $InvOrCredit . ' ' . _('Number') . ' ' . $FromTransNo;
        include('includes/header.php');
    ...
    if (!isset($FromTransNo) or $FromTransNo == '') {
        ...
        echo '<tr>
                  <td>' . _('Credit Note Reason') . '</td>
                   <td><select name="reason">
                     <option selected="selected" value="Customer Refund">' . _('Customer Refund') . '</option>
                    <option value="Items Damaged">' . _('Items Damaged') . '</option>
                    <option value="Wrong Supply">' . _('Wrong Supply') . '</option>
                    <option value="No Stock">' . _('No Stock') . '</option>

                    </select></td>
                        </tr>';

        echo '<tr>
                      <td>' . _('Corresponding invoice number to print') . '</td>
                                <td><input class="number" type="text" max="6" size="7" name="correspondingInvoiceNumber" /></td>
                        </tr>';
